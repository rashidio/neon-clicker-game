...
const TabsBar: React.FC<TabsBarProps> = ({ activeTab, onSelect }) => {
  return (
    <div className="max-w-6xl mx-auto px-4 sm:px-8 mt-6 sm:mt-8 mb-4 sm:mb-6">
      <div className="flex justify-center sm:justify-start gap-3 sm:gap-8 border-b border-white/10">
        {tabs.map((tab) => (
          <button
            key={tab.id}
            onClick={() => onSelect(tab.id)}
            className={`pb-2 sm:pb-3 text-sm sm:text-base font-light tracking-normal sm:tracking-wide transition-all relative flex items-center gap-2 sm:gap-2 ${
